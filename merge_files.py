...
output_file = os.path.join(output_file_folder,'imaginary_partner_patients.txt')
...
with open(output_file, 'w') as file:
    for patient in patient_list:        
        ...
        for study in uni_study_list:
            record = (df_radiology.loc[(df_radiology["pat_id"] == patient) & 
            (df_radiology["id"] == study)])
            if record.shape[0]>0:
        ...
        patho_list = []
        for study in study_list:
            patho_record = (df_pathology[(df_pathology["patient_id"]==patient) & 
            (df_pathology["study_id"] == study)])
